...
ソースからビルドする fallback は `cargo build --release --locked` を使うため、`Cargo.lock` と異なる依存解決は行いません。明示的にソースビルドしたい場合は `CAMPUS_LMS_BUILD_FROM_SOURCE=1` を設定します。
...
GitHub Releases には `campus-lms-v<version>-<platform>` と `.sha256` を置きます。`postinstall` は checksum を検証してから binary を配置します。checksum は破損検出用です。release workflow では build provenance attestation も生成します。署名検証を npm install 時に強制する仕組みは今後の強化対象です。

このリポジトリには Windows 用 prebuilt binary が含まれる場合があります。配布前には、対象 commit、`sha256`、`campus-lms --version`、`npm pack --dry-run` を確認してください。
...
- `campus-lms todo --days 14 --status-check-limit 20 --json`
- `campus-lms todo --days 14 --no-submission-status-check --json`
- `campus-lms todo --days 14 --include-undated --json`
...
- config には `base_url`, `username`, `service` などの接続設定が入ります。手動編集する場合、`base_url` は原則 HTTPS で、サブディレクトリを含む場合も末尾 `/` が必要です。
- ローカル開発用の HTTP は `allow_insecure_localhost = true` を明示した localhost 系URLだけ許可されます。
- cache にはコース名、課題名、締切、提出状態、明示的に開いた課題詳細本文が入る場合があります。
- `todo` / `ai snapshot` 用の課題一覧 cache では、課題本文 HTML と添付ファイルURLを保存しないようにしています。
- `cache_ttl_seconds` は「再取得するかどうか」の判定です。保存期間そのものではありません。
- `cache_retention_seconds` を過ぎた古い cache は、通常コマンド実行時に削除対象になります。デフォルトは30日です。
- `assignment show --no-cache` を使うと、課題詳細 cache を読まず、書き込みもしません。
...
`auth import-token` は、通常はOS credential storeへの保存・読み戻しだけを確認します。Moodle API上で実際に使えるかまで確認したい場合は `--live` を付けます。`--live` で疎通確認に失敗した場合は、tokenの保存だけ成功していても非0終了になります。

```bash
campus-lms auth import-token --base-url https://lms.example.ac.jp/moodle/ --username student123 --token-stdin --live --json
```
...
`todo` と `ai snapshot` は `--max-items` で返却件数を制限できます。`summary.returned_count` は実際に返した件数、`summary.total_matching_count` や `total_items_before_limit` は制限前の件数です。`ai snapshot` も `cache` metadata を返すため、AIはキャッシュ由来かどうかを判断できます。

`todo` は、calendarだけでは提出済み判定できない fallback 課題に対して、必要に応じて `mod_assign_get_submission_status` を呼びます。LMS負荷を抑えるため、デフォルトでは最大20件までです。増やす場合は `--status-check-limit N`、完全に止める場合は `--no-submission-status-check` を使います。締切なし課題は通常のToDoから外し、必要な場合だけ `--include-undated` で含めます。

`due_at` などの個別時刻は UTC の RFC3339 で出ます。`range.timezone` は日付範囲の計算に使うタイムゾーンです。
