...
ソースからビルドする fallback は `cargo build --release --locked` を使うため、`Cargo.lock` と異なる依存解決は行いません。
...
GitHub Releases には `campus-lms-v<version>-<platform>` と `.sha256` を置きます。`postinstall` は checksum を検証してから binary を配置します。checksum は破損検出用です。配布元そのものの真正性を強く保証するには、今後の release で署名や provenance を追加する方針です。
...
`auth import-token` は、通常はOS credential storeへの保存・読み戻しだけを確認します。Moodle API上で実際に使えるかまで確認したい場合は `--live` を付けます。
...
`todo` は、calendarだけでは提出済み判定できない fallback 課題に対して、必要に応じて `mod_assign_get_submission_status` を呼びます。LMS負荷を抑えるため、デフォルトでは最大20件までです。増やす場合は `--status-check-limit N`、完全に止める場合は `--no-submission-status-check` を使います。
